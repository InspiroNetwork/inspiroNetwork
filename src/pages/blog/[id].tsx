      ...
      <main className="pt-24 pb-16">
        ...
        <section className="relative py-16 px-4 mb-8 overflow-hidden">
          ...
          <div className="container mx-auto max-w-4xl relative z-10">
            <Link href="/blog" className="inline-flex items-center text-violet-300 hover:text-white transition mb-4">
        ...
        <section className="px-4">
          <div className="container mx-auto max-w-3xl">
            <div className="prose prose-lg prose-invert max-w-none">
              <p className="text-xl font-medium mb-6 text-violet-300">{post.excerpt}</p>

              {post.content.map((section, index) => (
                <div key={index} className="mb-8">
                  {section.heading && <h2 className="text-2xl font-montserrat font-bold mb-4">{section.heading}</h2>}
                  {section.paragraphs.map((paragraph, pIndex) => (
                    <p key={pIndex} className="mb-4 text-violet-200">
                      {paragraph}
                    </p>
                  ))}
                </div>
              ...
              <div className="mt-12 mb-8">
                <div className="flex flex-wrap gap-2">
                  {post.tags.map((tag, index) => (
                    <span
                      key={index}
                      className="inline-block px-3 py-1 bg-indigo-900/70 rounded-full text-xs text-violet-300"
                    >
                      #{tag}
                    </span>
                  ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-indigo-900/30 backdrop-blur-sm rounded-xl overflow-hidden border border-indigo-500/20">
                  ...
                  <div className="p-4">
                    ...
                    <a href="/blog/2" className="text-[#ffc83d] text-sm hover:underline">
                      Read More →
                    </a>
                  </div>
                </div>
                <div className="bg-indigo-900/30 backdrop-blur-sm rounded-xl overflow-hidden border border-indigo-500/20">
                  ...
                  <div className="p-4">
                    ...
                    <a href="/blog/3" className="text-[#ffc83d] text-sm hover:underline">
                      Read More →
                    </a>
                  </div>
                </div>
