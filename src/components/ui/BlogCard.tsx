    ...
    <div className="bg-indigo-900/30 backdrop-blur-sm rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition border border-indigo-500/20 hover-glow">
      ...
      <div className="p-6">
        <div className="flex items-center mb-2">
          <span className="text-xs text-violet-300">{format(new Date(post.publishedAt), "MMMM d, yyyy")}</span>
        ...
        <Link href={`/blog/${post.id}`} className="text-[#ffc83d] font-medium text-sm hover:underline">
          Read More →
        </Link>
      </div>
    </div>
