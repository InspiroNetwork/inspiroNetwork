...
const MetatronCube: React.FC<MetatronCubeProps> = ({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < petalCount; i++) {
      ...
      petalShape.ellipse(
        petalX * 0.5,
        petalY * 0.5, // Center of ellipse
        flowerRadius * 0.3,
        flowerRadius * 0.15, // x and y radius
        angle,
        angle - Math.PI,
        false, // Start and end angle
      )
    ...
    if (outerCircle) {
      ...
      for (let i = 0; i <= segments; i++) {
        ...
        circleVertices.push(outerRadius * Math.cos(theta), outerRadius * Math.sin(theta), 0)
      }
      ...
      for (let i = 0; i <= segments; i++) {
        const theta = (i / segments) * Math.PI * 2
        innerCircleVertices.push((outerRadius - 0.15) * Math.cos(theta), (outerRadius - 0.15) * Math.sin(theta), 0)
      }
    ...
    const animate = () => {
      ...
      if (cubeRef.current) {
        ...
        if (pulseEffect && !woodenStyle) {
          // No pulse in wooden style
          if (growing) {
            scale += 0.003
